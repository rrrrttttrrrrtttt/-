...
function toast({ ...props }: Toast) {
  ...
  interface ToastPropsWithId extends Toast {
    id: string;
    open: boolean;
  ...
  dispatch({
    type: "ADD_TOAST",
    toast: { ...props, id, open: true, onOpenChange: (open: boolean) => !open && dismiss() } as ToastPropsWithId,
  });

  return { id, dismiss, update };
}

function useToast(userId: number) {
  ...
  React.useEffect(() => {
    ...
    socket.on('notification', (notif: { message: string }) => {
      toast({ title: "Notification", description: notif.message }); // 自动触发 toast
    });
